...
ggplot(df_small, aes(X,Y)) +
 geom_point(aes(color=Keratin),size=0.3) +   
  theme_classic()
```
Let's look at an even smaller region. The first cell retrieved is 
```{r}
df_small[1,]
```
...
```{r}
nearest_neighbors$id[1,]
nearest_neighbors$dist[1,]
df_smaller$CellID[nearest_neighbors$id[1,]]
```
