...
ggplot(df_small, aes(X,Y)) +
 geom_point(aes(color=Keratin),size=0.3) +   
  theme_classic()
```
Let's look at an even smaller region, to check see the cell neighborhood. The first cell retrieved from the query above has a specific CellID
```{r}
cell_data <- df_small[1,]
cell_data
```
...
df_smaller <- df_small %>% filter(X > 6930 & X< 7030) %>% filter(Y > 21180 & Y< 21280)
df_smaller <- df_small %>% filter(X > xc-s & X< xc+s) %>% filter(Y > yc-s & Y< yc+s)
```

Counting rows, there are `r nrow(df_smaller)` cells within this region. 
...
For the cell of interest, the IDs of the nearest neighbors and distance (in pixels)
```{r}
kable(cbind(CellID=df_smaller$CellID[nearest_neighbors$id[1,]],Distance=nearest_neighbors$dist[1,]))
```
Now let's go back to the original square of 2500 pixels in each dimension and `r nrow(df_small)` cells. 

Calculate all pairwise distances among cell and find the (row) indices of the nearest neighbors of each cell.
```{r, cache=T}
k <- 10
...
kable(head(nearest_neighbors$id))
```
As before, this is in terms of index, but we can check that for the original cell of interest, the neighbor IDs and Distance are as before
For the cell of interest, the IDs of the nearest neighbors and distance (in pixels)
```{r}
cbind(CellID=df_small$CellID[nearest_neighbors$id[1,]],Distance=nearest_neighbors$dist[1,])
```
